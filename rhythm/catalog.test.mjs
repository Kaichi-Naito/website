...
test('sheet rows bind song details and assets; multiple difficulties have separate IDs',()=>{
  ...
  const songs=parseCatalog([row,hard]);assert.equal(songs.length,2);assert.equal(songs[0].midiPath,'rhythm/Rolling/Rolling_T4P_NORMAL.mid');assert.equal(songs[1].difficulty,'HARD');assert.equal(songs[1].midiPath,'rhythm/Rolling/Rolling_T4P_HARD.mid');assert.equal(songs[0].duration,125.952);
...
test('catalog preserves full song durations without modifying rows',()=>{
