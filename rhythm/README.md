...
画像は端末内で生成したPNGです。右上のジャケット（320px角）、難易度（36px）、同じ行の曲名（64px）とアーティスト（46px）、点数・ランク・精度・最大コンボ・判定内訳を収録し、名前やランキング登録情報は含めません。背景には実際のプレイ中のノーツが見えるフレームを使い、結果画面の背景にも適用します。空押しは画像のMISSに合算し、プレイ中のラベルもMISSに統一します（採点・ランキングの検証用データは従来どおり保持）。スコア画像は結果画面に常時表示し、保存・コピーの補助ボタンは表示しません。「Xに投稿」はコピーの成否にかかわらずリンクとしてXを開きます。navigator.share / canShare は使用しません。XのWeb Intentにローカル画像を自動添付する機能はありません。
