...
曲一覧の下、「スタート画面に戻る」の横の「練習モードにする」で練習を有効にして、0.5〜1倍速（0.1刻み）を選びます。その状態で各曲のNORMAL／HARDを押すと、選んだ難易度の練習が直接始まります。モードを解除すると通常プレイになります。スタート画面に戻ると練習モードは解除されます。初期値は0.7倍速、以降はそのページ内で最後の速度を保持します。音源と譜面を同じ倍率で変更し、入力判定の許容時間・音声オフセットは実時間のままです。0.5〜0.9倍速はSignalsmith Stretch 1.3.2（MIT）のWASM / AudioWorkletで音程を維持して再生します。1倍速・通常プレイは従来のAudioBufferSourceを使用します。補正処理は練習開始時だけ読み込み、音源長に比例する伸長済み音声を作らず、元のPCMをワークレットに転送してリアルタイム処理します。カウントイン中に遅延を補償した再生を予約し、音源のオフセットと譜面の実時間を同じ倍率で対応させます。曲末はAudioContextのゲイン予約で終了します。一時停止・復帰も同じ対応を保ちます。極端な低速では音の質感が変わる場合があります。
