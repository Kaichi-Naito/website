...
起動時はT4Pロゴと「スタート」を表示します。スタート後に曲選択とランキングを表示し、楽曲一覧の読み込みを開始します。スタートは楽曲のプレイボタンと同じ `song-play` イベント経由で `rhythm/song-selection.mp3` を鳴らします。通常クリック音は重ねません。結果画面の「曲選択に戻る」ではタイトル画面に戻さず、リロードで再びタイトル画面から始めます。
