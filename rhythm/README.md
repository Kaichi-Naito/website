...
結果画面の「Xに投稿」は、指定テンプレートの本文とスコア画像を準備します。共有メニューを使わず、PCはXの投稿画面、iPhone / iPadは `twitter://post?message=`、AndroidはXのパッケージを指定したIntentでアプリの投稿画面にリンクします。アプリ未導入・起動非対応の環境向けに、タップ後だけブラウザ用リンクを表示します。Android ChromeはIntent内のブラウザfallbackにも対応します。OS・ブラウザ・Xアプリのバージョンによって起動確認や非対応になる場合があるため、実機検証が必要です。画像コピーに対応した環境ではコピーも試みます（Xでの貼り付けが1回必要）。登録成功後、プレイIDで実順位が取得できた場合のみ順位の1行を追加します。投稿の確定はプレイヤーが行います。

画像は端末内で生成したPNGです。右上のジャケット、同じ行の曲名（64px）とアーティスト（46px）、点数・ランク・精度・最大コンボ・判定内訳を収録し、名前やランキング登録情報は含めません。背景には実際のプレイ中のノーツが見えるフレームを使い、結果画面の背景にも適用します。空押しは画像のMISSに合算し、プレイ中のラベルもMISSに統一します（採点・ランキングの検証用データは従来どおり保持）。スコア画像は結果画面に常時表示し、保存・コピーの補助ボタンは表示しません。「Xに投稿」はコピーの成否にかかわらずリンクとしてXを開きます。navigator.share / canShare は使用しません。XのWeb Intentにローカル画像を自動添付する機能はありません。
