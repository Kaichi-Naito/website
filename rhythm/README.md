...
起動時はT4Pロゴと「スタート」を表示します。スタート後に曲選択とランキングを表示し、楽曲一覧の読み込みを開始します。スタートは楽曲のプレイボタンと同じ `song-play` イベント経由で `rhythm/song-selection.mp3` を鳴らします。通常クリック音は重ねません。曲選択・一時停止・エラー・結果画面の「スタート画面に戻る」はタイトル画面に戻ります。再生中の音声と入力を止め、未完了の読み込みを無効化します。


## 練習モード

各曲の「練習する」で難易度と0.1〜1倍速（0.1刻み）を選びます。初期値は0.7倍速、以降はそのページ内で最後の速度を保持します。音源と譜面を同じ倍率で変更し、入力判定の許容時間・音声オフセットは実時間のままです。Web Audioの再生速度を変更するため低速では音程も下がります。開始前に画面で案内します。
