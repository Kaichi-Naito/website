...
曲一覧の上の「練習モードにする」で練習を有効にして、0.1〜1倍速（0.1刻み）を選びます。その状態で各曲のNORMAL／HARDを押すと、選んだ難易度の練習が直接始まります。モードを解除すると通常プレイになります。スタート画面に戻ると練習モードは解除されます。初期値は0.7倍速、以降はそのページ内で最後の速度を保持します。音源と譜面を同じ倍率で変更し、入力判定の許容時間・音声オフセットは実時間のままです。Web Audioの再生速度を変更するため低速では音程も下がります。開始前に画面で案内します。
