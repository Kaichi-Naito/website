...
結果画面の「Xに投稿」は、指定テンプレートの本文とスコア画像を準備します。スマホ・PCとも共有メニューを使わず、Xの投稿画面へ直接リンクします。画像コピーに対応した環境ではコピーも試みます（Xでの貼り付けが1回必要）。登録成功後、プレイIDで実順位が取得できた場合のみ順位の1行を追加します。投稿の確定はプレイヤーが行います。

画像は端末内で生成したPNGです。曲名・アーティスト・点数・ランク・精度・最大コンボ・判定内訳を収録し、名前やランキング登録情報は含めません。スコア画像は結果画面に常時表示し、保存・コピーの補助ボタンは表示しません。「Xに投稿」はコピーの成否にかかわらずリンクとしてXを開きます。navigator.share / canShare は使用しません。XのWeb Intentにローカル画像を自動添付する機能はありません。
